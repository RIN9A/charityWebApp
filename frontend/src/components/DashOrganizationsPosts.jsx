...
export default function DashOrganizationsPosts() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        ...
        if (currentUser.user.isAdmin) {
          res = await fetch(`/api/organization/post/getposts`);
        } else if (currentUser.user.isPersnOrg) {
          res = await fetch(
            `/api/organization/post/getposts?ogrn=${currentUser.user.id}`
          );
        }
    ...
    if (currentUser.user.isAdmin || currentUser.user.isPersnOrg) {
      fetchPosts();
    }
  }, [currentUser.user.id]);
  ...
  const handleShowMore = async () => {
    ...
    try {
      ...
      if (currentUser.user.isAdmin) {
        res = await fetch(
          `/api/organization/post/getposts?userId=${currentUser.user.id}&startIndex=${startIndex}`
        );
      } else if (currentUser.user.isPersnOrg) {
        res = await fetch(
          `/api/organization/post/getposts?ogrn=${currentUser.user.id}&startIndex=${startIndex}`
        );
      }
  ...
  const handleDeletePost = async () => {
    ...
    try {
      const res = await fetch(
        `/api/organization/deletepost/${postIdToDelete}/${currentUser.user.id}`,
        {
          method: "DELETE",
        }
      );
  ...
  return (
    ...
    >
      {currentUser.user.isPersnOrg && userPosts.length > 0 ? (
                    ...
                    <Table.Cell>
                      {post.status === "Опубликован" ? (
                        ...
                        <Link
                          ...
                          to={currentUser.user.isAdmin ? `/organization/update-post/${post.id}` : `/organization/update-post/${post.id}/${currentUser.user.id}`}
                          aria-disabled
                        >
                          <span>Изменить</span>
                        </Link>
        ...
        <p>Неопубликованные посты отсутствуют.</p>
      )}
      <Modal
        show={showModal}
