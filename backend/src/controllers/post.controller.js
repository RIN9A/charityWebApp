...
exports.create = async (req, res) => {
    ...
    try{
        const slug = req.body.title.split(' ').join('-').toLowerCase().replace(/[^\p{L}\p{N}-]/gu, '');       
        const post  = await db.query('INSERT INTO posts(id, "userId", content, title, slug) VALUES ($1, $2, $3, $4, $5) RETURNING *;', [id, req.user.id, req.body.content, req.body.title, slug]);
...
exports.getposts = async (req, res) => {
    try {
        ...
        if (req.query.userId) {
          ...
          const query = `
            SELECT * FROM posts  ${paramIndex !== 1 ? ` WHERE ${queryConditions.join(' AND ')}` :
          ...
          const lastMonthPosts = await db.query('SELECT COUNT(*) FROM posts WHERE "updatedAt" >= $1', [oneMonthAgo]);
